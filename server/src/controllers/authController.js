...
export async function loginAdmin(req,res){const result=await login(req.body.email,req.body.password,context(req));res.cookie('rr_refresh',result.refreshToken,cookieOptions);await audit(req,'AUTH_LOGIN','User',result.user.id,{},result.user.id);return ok(res,{accessToken:result.accessToken,user:result.user})}
